...
Helper functions in Rentrez allows you to get acquainted NCBI databases and their searchable fields.
It also allows you get updates on when was the last time that the database was updated.
...
The functions listed under this category allows you to use a search term or query to retrieve a list of article/object ids. 
This list of ids, will later allow you to retrieve partial or full summary records of that article/object. 
These functions permits you to to create dataframes in R, which allows easy data manipulation. 
...
This function allows you to find records that matches your keyword. 
...
Another important field is the ids, this field will have a list of ids that later will allow you to retrieve partial or full summary records. 
...
Now lets retrieve partial information about the records we collected in one of our searches. 
This function takes a vector of unique IDs or just one id.

But, first lets learn the syntax of the `entrez_summary()` function. 

`entrez_summary(db="database name", id=, rettype = "")`

**The `entrez_summary()` function takes 7 arguments but you don't need to use them all. The require ones are db, id and rettype.** 
**If you will be working with web_histories later, you don't need to use the id argument.**
...
#In this command we are giving a list of ids by using pcos_pm$ids. The $ operator extract a subset of a data object in R. 
...
Another difference  between `entrez_summary` and `entrez_fetch` is in the way both functions arrange the data in RStudio. `entrez_summary()` in most of the cases will return a data frame with elements that behave as lists, while `entrez_fetch()` will return a XML Document. 
