...
This week you will learn about NCBI Database API and how to access their data from RStudio.

For the last few weeks, you have been working on data that you have downloaded and manage to access it locally using RStudio. Now we are shifting the gears to work with data that is not located in our computers but is on the internet. 

For this lesson, we are going to use `Rentrez` to get data directly from the PubMed database.

# The Data

For this week, we will be working with semi-structured data. This type of data lacks of predefined community metadata structures. Also, it is the type of data that does not conforms to any specific schema such as MODS, METS, BIBFRAME, EAD or others. 

Our data will be located in the National Center for BioInformation (NCBI) Database. 

The NCBI Database has a XML proprietary format, meaning that we rely on [Entrez](https://www.ncbi.nlm.nih.gov/books/NBK25497/?report=reader) documentation and [tutorials](https://cran.r-project.org/web/packages/rentrez/vignettes/rentrez_tutorial.html) to understand how to access information. 

For this week, we need to learn how to perform search queries to the NCBI Entrez API using `Rentrez`. Also, we need to learn how to transform this data into a usable format. 
...
`Rentrez` is an R interface that allows its users to interact with NCBI API, such as PubMed, GenBank, GEO and more. With `Rentrez`, you do not need to use any additional program or terminal to access NCBI Data. This means that you can request data from multiple databases in the same RStudio Session. 
