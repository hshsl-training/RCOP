...
All geoms have certain visual attributes that can be modified. Polygons like bars, have the properties fill and color. You can change the inside color of a bar with fill, and the outside with color.
...
::: {.callout-note}
How did we know the color names "blue" and "red would work in the code above? R has several built in color names. You can see them by calling the function `colors()`. You can also specify colors using rgb and hexadecimal codes. 
:::
